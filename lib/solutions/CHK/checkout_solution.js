  ...
  checkout(skus) {
    ...
    for (const [item, quantity] of Object.entries(quantities)) {
      if (discounts[item]) {
        ...
        total += prices[item] * (quantity - discountedQuantity * )
      }

      if (item === "A") {
        const discountedQuantity = Math.floor(quantity / 3);
        total += 130 * discountedQuantity;
        total += prices[item] * (quantity - discountedQuantity * 3);
      } else if (item === "B") {
      } else total += prices[item] * quantity;
    }
